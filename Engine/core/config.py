...
def resource_path(relative: str) -> Path:
    if getattr(sys, 'frozen', False):
        ...
        # Both source and Tauri resources retain Engine/core/config.py.
        # sys.executable points to the embedded Python in Engine/bin/python,
...
def find_binary(name: str) -> str:
    ...
    # installing them. Prefer resources shipped with this Engine instance.
    search_dirs = [
        base / "bin",  # Engine/bin, including Tauri's resource mapping
        base,          # PyInstaller may place binaries directly in _MEIPASS
        base.parent,   # Windows Tauri sidecars next to the Engine directory
    ...
    if name.lower() in ("vspipe", "vspipe.exe"):
        ...
        # Python, before any older standalone media binary in Engine/bin.
