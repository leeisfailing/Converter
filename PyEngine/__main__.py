"""PyEngine entry point - JSON-RPC command dispatcher."""
import json
import sys
import traceback
...
from PyEngine.core.security import validate_json_value
...
HANDLERS = {
    ...
    "start_transcoder": ("reduce", "handle_start_transcoder"),
    "start_upscale": ("upscale", "handle_start_upscale"),
    "start_enhance": ("enhance", "handle_start_enhance"),
}
...
def _load_handler(cmd):
    ...
    return getattr(import_module(f"PyEngine.handlers.{module}"), name)
...
def _has_active_worker():
    for key, worker in list(_current_worker.items()):
        ...
        if thread is not None:
            ...
            # thread exits. Give that thread a bounded chance to finish.
        ...
        try:
            ...
            if cmd == "cancel":
                ...
                try:
                    ...
                    continue
                if cmd.startswith("start_") and _has_active_worker():
                    send_response({"ok": False, "error": "An operation is already in progress"})
                    continue
