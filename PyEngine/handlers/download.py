...
from PyEngine.core.ipc import send_progress, send_finished, send_download_status
from PyEngine.core.security import validate_url, validate_output_dir, validate_string, validate_download_format
from PyEngine.workers.downloader import DownloadWorker
from PyEngine.workers.tiktok import TikTokDownloadWorker
from PyEngine.core.tiktok import is_tiktok_url
...
def handle_start_download(cmd_args: dict):
    ...
    if format_type in ('tiktok', 'tiktok_no_watermark') and not tiktok:
        send_finished(False, 'TikTok download options require a TikTok link', '')
        return None
    if tiktok and format_type not in ('tiktok', 'tiktok_no_watermark'):
        format_type = 'tiktok'
    worker_class = TikTokDownloadWorker if tiktok else DownloadWorker
    worker = worker_class(
