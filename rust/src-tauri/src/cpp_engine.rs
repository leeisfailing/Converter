//! C++ GPU engine bridge — same JSON-lines protocol as the Python engine,
//! but spawns the native `gpu_engine` binary for lower-latency GPU work.
...
pub struct CppEngine {
    active: Mutex<std::collections::HashMap<String, ActiveCppProcess>>,
}

impl CppEngine {
    /// Kill any running C++ engine process immediately.
...
/// Resolve the C++ engine binary path. Returns `None` if not found.
...
pub fn binary_path_in(resource_dir: Option<&std::path::Path>) -> Option<String> {
    ...
    let names: Vec<String> = {
        ...
        if let Some(triple) = sidecar_triple() {
            v.push(format!("{base}-{triple}"));
        }
        v
    };
...
fn sidecar_triple() -> Option<String> {
...
#[inline]
async fn reap(mut process: ActiveCppProcess) {
    drop(process.stdin);
...
/// One-shot request: spawn, send, close stdin, read response.
pub async fn request<T: DeserializeOwned>(app: &AppHandle, value: serde_json::Value) -> Result<T, String> {
    let mut child = command(app)?.spawn().map_err(|e| format!("Failed to start C++ engine: {e}"))?;
    ...
    let mut stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await { result.push(line); }
        result
    });
    ...
    let stdout_lines = match stdout_result {
        ...
        Err(_) => {
            ...
            stdout_task.abort();
            stderr_task.abort();
            return Err(format!("C++ engine request timed out after {} seconds", ENGINE_TIMEOUT_SECS));
        },
    };
    ...
    for line in &stdout_lines {
        ...
        if let Ok(value) = serde_json::from_str::<serde_json::Value>(line) {
            if value.get("ok").and_then(|v| v.as_bool()) == Some(false) {
                ...
                let msg = value.get("error").and_then(|v| v.as_str()).unwrap_or("C++ engine request failed");
    ...
    let stderr = stderr_task.await.unwrap_or_default();
    Err(format!("C++ engine returned no valid response: {stderr}"))
}

/// Interactive streaming: spawn, send, keep stdin open for cancel, stream progress.
...
pub async fn run_interactive_command(app: AppHandle, value: serde_json::Value, prefix: &str, id: String) -> Result<(), String> {
    ...
    let state = app.state::<CppEngine>();
    let mut child = command(&app)?.spawn().map_err(|e| format!("Failed to start C++ engine: {e}"))?;
    let mut stdin = child.stdin.take().ok_or("Missing engine stdin")?;
    let stdout = child.stdout.take().ok_or("Missing engine stdout")?;
    let stderr = child.stderr.take().ok_or("Missing engine stderr")?;
    send(&mut stdin, &value).await?;
    state.active.lock().await.insert(id.clone(), ActiveCppProcess { child, stdin });
            ...
            Ok(Some(line)) => {
                ...
                match value.get("type").and_then(|v| v.as_str()) {
                    ...
                    _ => if value.get("ok").and_then(|v| v.as_bool()) == Some(false) {
                        break Err(value.get("error").and_then(|v| v.as_str()).unwrap_or("C++ engine command failed").to_string());
                    },
                }
            }
            Ok(None) => break Err("C++ engine exited before completing the operation".into()),
            Err(error) => break Err(format!("C++ engine output read error: {error}")),
        }
    };
...
#[tauri::command]
pub async fn cancel_cpp_operation(app: AppHandle) -> Result<(), String> {
